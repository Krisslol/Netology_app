package com.example.netology

import androidx.appcompat.app.AppCompatActivity
import android.os.Bundle
import com.example.netology.databinding.ActivityMainBinding
import com.example.netology.dto.Post
import com.example.netology.dto.displayNumbers


class MainActivity : AppCompatActivity() {
    override fun onCreate(savedInstanceState: Bundle?) {
        super.onCreate(savedInstanceState)
        val binding = ActivityMainBinding.inflate(layoutInflater)
        setContentView(binding.root)

        val post = Post(
            id = 1,
            author = "Нетология. Университет интернет-профессий будущего",

            published = "21 мая в 18:36",
            content = "Привет, это новая Нетология! Когда-то Нетология начиналась с интенсивов по онлайн-маркетингу. Затем появились курсы по дизайну, разработке, аналитике и управлению. Мы растём сами и помогаем расти студентам: от новичков до уверенных профессионалов. Но самое важное остаётся с нами: мы верим, что в каждом уже есть сила, которая заставляет хотеть больше, целиться выше, бежать быстрее. Наша миссия — помочь встать на путь роста и начать цепочку перемен → http://netolo.gy/fyb",
            likedByMe = false,
            likes = 9999,
            reposts = 9999,
            repostedByMe = false,
            views = 9999,
            viewedByMe = false
        )

        with(binding) {
            author.text = post.author
            published.text = post.published
            content.text = post.content
            numberLike.text = post.likes.toString()
            numberRepost.text = post.reposts.toString()
            numberView.text = post.views.toString()

            if (post.likedByMe) {
                like.setImageResource(R.drawable.ic_baseline_favorite_24)
            }


            like.setOnClickListener {
                post.likedByMe = !post.likedByMe
                if (post.likedByMe) post.likes++ else post.likes--
                like.setImageResource(if (post.likedByMe) R.drawable.ic_baseline_favorite_24 else R.drawable.ic_baseline_favorite_border_24)
                numberLike.text = post.likes.toString()
                numberLike.text = displayNumbers(post.likes.toLong())
            }
            repost.setOnClickListener {
                post.repostedByMe = !post.repostedByMe
                if (post.repostedByMe) post.reposts++
                numberRepost.text = post.reposts.toString()
                numberRepost.text = displayNumbers(post.reposts.toLong())
            }

            view.setOnClickListener {
                post.viewedByMe = !post.viewedByMe
                if (post.viewedByMe) post.views++
                numberView.text = post.views.toString()
                numberView.text = displayNumbers(post.views.toLong())

            }
        }
    }
}
